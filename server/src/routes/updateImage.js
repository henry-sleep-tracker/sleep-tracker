...
router.put('/:id', async (req, res)=>{
  ...
  try { 
    ...
    console.log("USERFOUND ID", userFoud.imageId);
    if(!userFoud){
    ...
    if(!file && userFoud.image){
        ...
        const update = await User.update({ image: null, imageId: null }, {
          where: {
            id: id
          }
        });
    ...
    if(file){
      if(!userFoud.image){
        ...
        const update = await User.update({ image: result.secure_url, imageId: result.public_id }, {
          where: {
            id: id
          }
        });
